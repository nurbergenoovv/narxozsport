...
import ListSchedulesCheckCard from '../../components/CardComponents/ListSchedulesCheckCard'
...
export default function SStudentVisitCheck({ navigation }) {
	...
	const fetchSections = async () => {
		try {
		    setIsLoading(true)
			const response = await Sections.getStudentCheckList()
	...
	return (
		<SafeAreaView className='flex-1 items-center pt-2'>
			...
			<HeaderGoBack pageName={'Журнал'} navigation={navigation} />
				...
				<View className='pb-7 pt-4'>
					{data.map(section => (
						<View key={section.day}>
							...
							<View style={{ gap: 10 }} className='mb-4 mt-2'>
								{section.schedules.map((Schedule, index2) => (
									<ListSchedulesCheckCard
										data={Schedule}
										key={index2}
